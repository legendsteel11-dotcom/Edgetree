...
- **접기/펼치기**: 앱 아이콘 클릭 한 번으로 아이콘 바만 남는 접힘 상태로
  ...
  자동으로 펼쳐지고, 기본값으로는 마우스가 벗어나면 다시 조용히 숨겨짐
  (옵션 메뉴의 "즉시 자동 숨김"을 끄면 대신 바깥을 클릭해야 닫히는 방식으로
  전환 가능). 숨겨졌을 때 남는 얇은 바의 두께도 옵션 메뉴에서 3~8px로 조절
  가능(펼쳐진 상태에서 핀 아이콘을 누르면 오토하이드를 끄고 고정할 수 있음).
  모두 부드러운 폭 애니메이션과 함께 전환
