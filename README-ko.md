...
- **폴더 검색** (`Ctrl+F` 또는 제목표시줄 검색 아이콘으로 트리 ↔ 검색 화면
  전환): 폴더를 지정하면 그 폴더를 메모리에 인덱싱해 파일명을 즉시 찾음 —
  부분일치 또는 `*`/`?` 와일드카드, 결과는 폴더별로 묶여 표시되고 이름/날짜로
  정렬 가능. 결과를 클릭하면 트리에서 해당 위치로 이동(폴더가 맨 위에 고정)
  하거나 다른 앱으로 바로 끌어다 쓸 수 있고, 최근 검색어를 기억함(`↑`/`↓`로
  불러오고 개별 삭제 가능). `Ctrl+E`로 트리로 복귀
