...
- **화면 가장자리 고정 배치** — 기본은 좌측, 옵션 메뉴에서 우측으로도 전환
  ...
  Alt+Tab/작업표시줄에서 제외됨
- **오토하이드**: 앱 아이콘 클릭 한 번으로 화면 가장자리에 얇은 바만 남기고
  완전히 숨겨짐 — 마우스를 가장자리로 가져가면 자동으로 펼쳐지고,
  기본값으로는 마우스가 벗어나면 다시 조용히 숨겨짐(옵션 메뉴의 "즉시 자동
  숨김"을 끄면 대신 바깥을 클릭해야 닫히는 방식으로 전환 가능). 숨겨졌을 때
  남는 얇은 바의 두께도 옵션 메뉴에서 3~8px로 조절 가능(펼쳐진 상태에서 핀
  아이콘을 누르면 오토하이드를 끄고 고정할 수 있음). 부드러운 폭
  애니메이션과 함께 전환
...
- **아이콘 방식 2택**: 기본은 Material Icon Theme 기반 파일/폴더 아이콘
  ([THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md) 참고), 옵션 메뉴의
  "아이콘 방식"에서 **윈도우 탐색기와 동일한 아이콘**으로 전환 가능(재시작
  불필요, 트리·검색 결과·즐겨찾기 공통 적용). exe/바로가기 등은 탐색기처럼
  그 파일 고유의 아이콘이 표시되고, 아이콘 추출은 백그라운드에서 처리되어
  네트워크 드라이브에서도 느려지지 않음; 드라이브 이름은 굵게 표시
